...
class VerifyPaymentView(APIView):
    ...
    def post(self, request):
        ...
        try:
            # Always verify the payment signature - never bypass, even in DEBUG
            ...
            with transaction.atomic():
                ...
                # Prevent processing already successful orders
            ...
            print(f"ERROR: Unexpected Payment Verification Error: {str(e)}")
            order = PaymentOrder.objects.filter(
                razorpay_order_id=razorpay_order_id
            ).first()
            if order:
        ...
        try:
            ...
            if event == "payment.captured":
                ...
                with transaction.atomic():
                    ...
                    order = (
                        PaymentOrder.objects.filter(razorpay_order_id=order_id)
                        .select_for_update()
                        .first()
                    )
...
class AdminStatsView(APIView):
    ...
    def get(self, request):
        ...
        # Count unique users who were ever on trial (including those who later paid)
        ever_paid_users_count = UserSubscription.objects.filter(
            purchase_count__gt=0
        ).count()
        total_trial_users_count = (
            UserSubscription.objects.filter(plan__name__icontains="Trial")
            .distinct()
            .count()
        )
        ...
        churn_count = UserSubscription.objects.filter(
            purchase_count__gt=0, is_active=False
        ).count()
        churn_rate = 0
